...
def get_media_generator():
    local_cur.execute("""SELECT * FROM (
        SELECT substring(url from 'https?://[^/]*/'), count(*) FROM (
            SELECT media.url, media_objects.etag FROM media LEFT JOIN media_objects ON media.url = media_objects.url WHERE type IS NOT NULL AND (last_status IS NULL or (last_status >= 400 and last_check < now() - '1 month'::interval) and media.url like 'http://sweetgum.nybg.org/%')
    ...
    sweetgum = 'http://sweetgum.nybg.org/%'
    for sub_row in subs_rows:
        ...
        ) AS a WHERE a.etag IS NULL""", (sweetgum,))
